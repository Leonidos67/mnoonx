...
export interface SiriOrbProps {
  ...
  className?: string;
  /** Stronger contrast & color for icon/nav sizes (e.g. sidebar) */
  compact?: boolean;
  colors?: {
    bg?: string;
    c1?: string;
...
const SiriOrb: React.FC<SiriOrbProps> = ({
  ...
  let maskRadius = getMaskRadius(sizeValue);
  ...
  let finalContrast = getFinalContrast(sizeValue);
  let resolvedBlur = blurAmount;
  let resolvedShadow = shadowSpread;

  if (compact && sizeValue < 64) {
    resolvedBlur = Math.max(0.5, Math.min(resolvedBlur, 1.2));
    finalContrast = Math.max(finalContrast, 2.6);
    resolvedShadow = Math.max(resolvedShadow, 2.5);
    maskRadius = sizeValue < 42 ? '10%' : '18%';
  }

  return (
    <div
      className={`siri-orb ${className}`.trim()}
      style={
        {
          width: size,
          height: size,
          ...
          '--blur-amount': `${resolvedBlur}px`,
          '--contrast-amount': finalContrast,
          '--dot-size': `${dotSize}px`,
          '--shadow-spread': `${resolvedShadow}px`,
