...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    float sPix=clamp(yPix/max(R_V*uVLenFactor,EPS),0.0,1.0),topA=pow(1.0-smoothstep(TOP_FADE_START,1.0,sPix),TOP_FADE_EXP);
    float L=a+b*topA;
...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    // Opaque black + beam (official React Bits LaserFlow). Transparent mode often hides the laser.
    ...
    const resizeKick = window.setTimeout(() => {
      lastSizeRef.current = { width: 0, height: 0, dpr: 0 };
      setSizeNow();
    }, 80);

    return () => {
      window.clearTimeout(resizeKick);
